...
var Status;
(function (Status) {
    Status[Status["VISITED"] = 1] = "VISITED";
    Status[Status["EXPLORED"] = 2] = "EXPLORED";
})(Status || (Status = {}));
...
function neighbors(grid, cell) {
    ...
    return neighbors.filter((neighbor) => neighbor[0] >= 0 && neighbor[0] < grid.length && neighbor[1] >= 0 && neighbor[1] < grid[0].length);
}
...
function generateMaze(width, height) {
    ...
    const walls = Array.from({ length: width }, () => Array.from({ length: height }, () => {
        return {
            top: true,
            right: true,
            bottom: true,
    ...
    while (stack.length > 0) {
        ...
        const unvisited = neighbors(grid, currentCell).filter((neighbor) => (grid[neighbor[0]][neighbor[1]] & Status.VISITED) == 0);
...
function findSolution(maze, start, end) {
    const parentAndDist = Array.from({ length: maze.grid.length }, () => Array.from({ length: maze.grid[0].length }, () => [-1, -1, Infinity]));
    const queue = [];
    queue.push(start);
    ...
    while (queue.length > 0) {
        ...
        neighborCells.forEach((neighbor) => {
            ...
            if ((dx == 1 && maze.walls[current[0]][current[1]].right) ||
